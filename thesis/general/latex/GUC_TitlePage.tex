%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\thispagestyle{empty}
\begin{center}
	\hrule
	%\vspace{0.01cm}
	%\hrule
	\vspace{1cm}
	{\Large \bf Media Engineering and Technology Faculty}\\ %[1mm]
	\vspace{0.3cm}
	{\Large \bf German University in Cairo}\\ %[1mm]
	\begin{figure}[htb]
		\centering
		\includegraphics[width=2cm]{logo-university-guc.pdf}
		%\label{GUC_LOGO}
	\end{figure}
	
	\vspace{0.5cm}
	
	{\Large \bf Institute of Visualization and Interactive Systems}\\ %[1mm]
	\vspace{0.3cm}
	{\Large \bf University of Stuttgart}\\ %[1mm]
	
	\begin{figure}[htb]
		\centering
		\includegraphics[width=2cm]{logo-university-stuttgart.pdf}
		%\label{STUTTGART_LOGO}
		\includegraphics[width=2cm]{logo-university-vis.pdf}
		%\label{VIS_LOGO}
	\end{figure}
	
	\vspace{2cm}
	
	{\Large \bf \titleOfThesisOne}\\
	\vspace{1cm}
	
	{\large \bf \typeOfThesis}\\
	\vspace{4cm}
	
	\parbox{2.5cm}{
		%\begin{large}
			\begin{tabbing}
				Author: \hspace{2cm}
					\=\authorOfThesis\\[2mm]
				Supervisor: 
					\>\supervisorOne\\[2mm]
					\>\supervisorTwo\\[2mm]
				Reviewers: 
					\>\reviewerOne\\[1mm]
				Submission Date: 
					\>\submissionDate\\
			\end{tabbing}
		%\end{large}
	}\\
	\hrule
	%\vspace{0.01cm}
	%\hrule
\end{center}
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
