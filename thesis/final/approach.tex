...
Simple command based editing was implemented in the prototype. It has a set of simple commands used to create the required diagram. A context free language understandable by the script engine in the prototype is presented in \reffig{script_language_grammar}. This language is a very sortened form of SVG [ABBREVIATION], all extra information and attributes like ``width='' and ``height='' were removed. The language only contains commands for the supported types by the prototype but this can easily be extended in the case of using it with another diagramming tool.
...
\item Rectangle $\rightarrow$ rectangle int int
\item Ellipse $\rightarrow$ ellipse int int
\item Triangle $\rightarrow$ triangle int int
...
\item X $\rightarrow$ int
\item Y $\rightarrow$ int
\item Width $\rightarrow$ int
\item Height $\rightarrow$ int
...
[CHANGE THE FIGURE TO A REAL DIAGRAM]
